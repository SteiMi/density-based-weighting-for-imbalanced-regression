...
CHECKPOINT_DIR = os.path.expanduser(os.path.join(config.get('SRCNN', 'scratch'), DEEPSD_MODEL_NAME, 'models',
                                    "srcnn_%s_%s_%s" % ('%s',
...
def freeze_graph(model_folder, graph_name=None):
    ...
    with tf.Session() as sess:
        ...
        if graph_name is not None:
            # The following might have changed with a newer tensorflow? not sure
            output_node_names = "prediction_1"
        ...
        for node in gd.node:
            if node.op == 'RefSwitch':
                ...
                for index in range(len(node.input)):  # changed for python3
...
def main(frozen_graph, output_node, year, scale1=1., n_stacked=1):
    ...
    precip = xr.DataArray(downscaled[:,:,:,0], coords=[times, lats[0], lons[0]], 
                          dims=['time', 'lat', 'lon'])
    xr.Dataset({'precip': precip}).to_netcdf(os.path.join(OUTPUT_DIR,  "precip_%i_downscaled.nc" % year))
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'res.pdf'))
    plt.close()
