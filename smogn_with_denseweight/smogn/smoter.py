...
from smogn.denseweight import TargetRelevance


def set_size(width: float = 347.0, fraction: float = 1., subplot=[1, 1]):
...
def smoter(
    ...
    rel_method = "auto",      ## relevance method ("auto", "manual" or "denseweight")
    rel_xtrm_type = "both",   ## distribution focus ("high", "low", "both")
    rel_coef = 1.5,           ## coefficient for box plot (pos real)
    rel_alpha = 1.5,          ## coefficient for DenseWeight's alpha (pos real,
                              ## only for denseweight)
    ...
    if rel_method == "denseweight":
        dw = TargetRelevance(np.reshape(y_sort.values, (y_sort.values.shape[0], 1)), alpha=rel_alpha)
        # Denseweight is not bound to [0, 1] but [0, inf) which is why we use norm_relevances
        y_phi = dw.norm_relevances
    else:
        ## -------------------------------- phi --------------------------------- ##
        ...
        color = 'tab:blue'
        fig, ax1 = plt.subplots(figsize=set_size(fraction=0.4))
        ax1.hist(y_sort, density=True, color=color)
        ax1.set_ylabel('$p(y)$', color=color)
        ax1.set_xlabel('$y$')
        ...
        color = 'tab:orange'
        ax2.plot(y_sort, y_phi, color=color)
        ax2.set_ylabel('$\\phi(y)$', color=color)
        ax2.tick_params(axis='y', labelcolor=color)
        plt.savefig('smogn_phi_relevance.pdf', format='pdf', bbox_inches='tight')
